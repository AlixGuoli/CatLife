...
object Utils {
    ...
    fun getRemoteDnsServers(): List<String> {
        val remoteDns = settingsStorage?.decodeString(AppKey.PREF_REMOTE_DNS) ?: AppKey.DNS_AGENT
        val ret = remoteDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(AppKey.DNS_AGENT)
        }
        return ret
    }

    fun getVpnDnsServers(): List<String> {
        val vpnDns = settingsStorage?.decodeString(AppKey.PREF_VPN_DNS)
            ?: settingsStorage?.decodeString(AppKey.PREF_REMOTE_DNS)
            ?: AppKey.DNS_AGENT
    ...
    fun getDomesticDnsServers(): List<String> {
        val domesticDns = settingsStorage?.decodeString(AppKey.PREF_DOMESTIC_DNS) ?: AppKey.DNS_DIRECT
        val ret = domesticDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(AppKey.DNS_DIRECT)
        }
        return ret
    }
...
//    }
    ...
    fun stopVService(context: Context) {
        ...
        MessageUtil.sendMsg2Service(context, AppKey.MSG_STATE_STOP, "")
    }
    ...
    fun userAssetPath(context: Context?): String {
        if (context == null)
            return ""
        val extDir = context.getExternalFilesDir(AppKey.DIR_ASSETS)
            ?: return context.getDir(AppKey.DIR_ASSETS, 0).absolutePath
    ...
    fun getLocale(context: Context): Locale =
        when (settingsStorage?.decodeString(AppKey.PREF_LANGUAGE) ?: "auto") {
            "auto" ->  getSysLocale()
            "en" -> Locale("en")
            "zh-rCN" -> Locale("zh", "CN")
            "zh-rTW" -> Locale("zh", "TW")
